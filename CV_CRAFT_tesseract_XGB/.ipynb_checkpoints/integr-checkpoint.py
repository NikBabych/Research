...
def detect_save_text(image,net):
    ...
    bboxes, polys, score_text = test_net(net, image, text_threshold=0.5,link_threshold=0.7, low_text=0.2, poly=False)
...
def preproc_mean_text(labels,img):
    ...
    without_text = cv2.morphologyEx(without_text, cv2.MORPH_DILATE, kernel, iterations=1)
    return without_text, mean_text_list
...
def get_df(img_path, predict_dict, label_dict):
    ...
    for ind, rect in predict_dict.items():
        ...
        for ind2, rect_2 in label_dict.items():
            ...
            if intersec > 0.88:
                ...
                if ind not in df_intersec:
                    try:
                        qwe = pytesseract.image_to_string(
                            img[coord[1]:coord[1] + coord[3], coord[0]:coord[0] + coord[2]],
                            config='')
                    ...
                    if (amount) == 1:
                        ...
                        qwe = 1 if len(qwe) > 0 else 0

                    df_intersec[ind] = (
                        coord[0], coord[1], coord[2], coord[3], coord_label[4], intersec, contourArea_rect(coord),
                        img_path, qwe, amount)
                    indexs.append(ind)
    for i in predict_dict.keys():
        ...
        df_intersec[i] = (coord[0], coord[1], coord[2], coord[3], 5, 0, contourArea_rect(coord), img_path, qwe, amount)
    return df_intersec
